...
ggplot(frequency_combined, aes(x = proportion, y = `G20-G8`,
                      color=abs(`G20-G8`-proportion)))+
...
```{r}
#correlation between words in different texts
cor.test(data=frequency_combined[frequency_combined$country=="G8",], 
         ~ proportion + `G20-G8`)
```
...
ggplot(frequency_bigram, aes(x = proportion, y = `G20-G8`,
                      color=abs(`G20-G8`-proportion)))+
  geom_abline(color="gray40", lty=2)+
  geom_jitter(alpha=0.1, size=2.5, width=0.3, height=0.3)+
...
```
```{r}
plot_grid(network_g8, network_g20_g8, labels = c('G8', 'G20-G8'), label_size = 16, scale = 0.95, label_colour = "Black")
